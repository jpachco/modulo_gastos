...
'CompraPPTO'AS BASE,
  ...
  FROM presupuesto_ro 
  WHERE Marca in('Calderoni','calderonic','roberts','roberts re','om','robactive','G.VALENTIN','GEOX')
  and Familia NOT IN( 'ACCESORIOS', 'BERMUDA', 'BLAZER D', 'BLUSA', 'BOLSA', 'BORDADO', 'CAMISA', 'CHALECOD', 'CINTA', 'CONJUNTO', 'COOR/PANTS', 'CORB&MAN', 'CORBATERO', 'EMPAQUE', 'EQUIPAJE', 'FAJA&MONO', 'FALDA', 'FALDA DAMA', 'GORRA', 'HABILITACION', 'KITSMOKREN', 'MAQUILA', 'PANTALON', 'PANTALON D', 'PANTALON DAMA', 'PANTALOND', 'PAQUETE', 'PIJAMA', 'ROPA INT', 'SACO', 'SACO DAMA', 'SLACK', 'SUDADERA', 'TELA', 'TELAS', 'VARIOS', 'BLUSAS', 'CAJA', 'CAMISA CER', 'CUBREPOLVO', 'DONATIVO', 'ENVIO', 'GORRA', 'JOYERIA', 'KEPI', 'KIT ACCESO', 'MASCADA', 'PANTALONVD', 'PORTAFOLIO', 'PORTATRAJE', 'REGALOS', 'SACO SPD', 'SOLAPAS', 'SOMBREROS', 'UNIFORMES')
...
'VentaPPTO'AS BASE,
  ...
  FROM presupuesto_ro
  WHERE Marca in('Calderoni','calderonic','roberts','roberts re','om','robactive','G.VALENTIN')
  and Familia NOT IN( 'ACCESORIOS', 'BERMUDA', 'BLAZER D', 'BLUSA', 'BOLSA', 'BORDADO', 'CAMISA', 'CHALECOD', 'CINTA', 'CONJUNTO', 'COOR/PANTS', 'CORB&MAN', 'CORBATERO', 'EMPAQUE', 'EQUIPAJE', 'FAJA&MONO', 'FALDA', 'FALDA DAMA', 'GORRA', 'HABILITACION', 'KITSMOKREN', 'MAQUILA', 'PANTALON', 'PANTALON D', 'PANTALON DAMA', 'PANTALOND', 'PAQUETE', 'PIJAMA', 'ROPA INT', 'SACO', 'SACO DAMA', 'SLACK', 'SUDADERA', 'TELA', 'TELAS', 'VARIOS', 'BLUSAS', 'CAJA', 'CAMISA CER', 'CUBREPOLVO', 'DONATIVO', 'ENVIO', 'GORRA', 'JOYERIA', 'KEPI', 'KIT ACCESO', 'MASCADA', 'PANTALONVD', 'PORTAFOLIO', 'PORTATRAJE', 'REGALOS', 'SACO SPD', 'SOLAPAS', 'SOMBREROS', 'UNIFORMES')
...
--declare @MARCAS2 nvarchar(max) = 'IN  (''Calderoni'',''calderonic'',''roberts'',''roberts re'',''om'',''robactive'',''G.VALENTIN'',''GEOX'')'
declare @MARCAS2 nvarchar(max) = '  like ''%''   '
declare @FAMILIA2 nvarchar(max) = ' IN (
''CALCETIN'',
''PANTALON V'',
''SEPARPANTV'',
''TRAJE'',
...
where [Fecha recepción esperada]>=''20180101''
  ...
  and pl.Colección  ' + @MARCAS2 + '
  and Familia '+ @FAMILIA2 +'
...
select Año as Año, Categoria,''Venta''AS BASE,Familia,Colección as Marca,
  ...
  FROM VtaFamMesSOP
  where año>=2018 and [Colección] ' + @MARCAS2 + '
...
declare @MARCAS2 nvarchar(max) = 'IN  (''Calderoni'',''calderonic'',''roberts'',''roberts re'',''om'',''robactive'',''G.VALENTIN'',''GEOX'',''HUGO BOSS'')'
declare @FAMILIA2 nvarchar(max) = 'NOT IN( ''ACCESORIOS'', ''BERMUDA'', ''BLAZER D'', ''BLUSA'', ''BOLSA'', ''BORDADO'', ''CAMISA'', ''CHALECOD'', ''CINTA'', ''CONJUNTO'', ''COOR/PANTS'', ''CORB&MAN'', ''CORBATERO'', ''EMPAQUE'', ''EQUIPAJE''
  , ''FAJA&MONO'', ''FALDA'', ''FALDA DAMA'', ''GORRA'', ''HABILITACION'', ''KITSMOKREN'', ''MAQUILA'', ''PANTALON'', ''PANTALON D'', ''PANTALON DAMA''
  , ''PANTALOND'', ''PAQUETE'', ''PIJAMA'', ''ROPA INT'', ''SACO'', ''SACO DAMA'', ''SLACK'', ''SUDADERA'', ''TELA'', ''TELAS'', ''VARIOS'', ''BLUSAS''
  , ''CAJA'', ''CAMISA CER'', ''CUBREPOLVO'', ''DONATIVO'', ''ENVIO'', ''GORRA'', ''JOYERIA'', ''KEPI'', ''KIT ACCESO'', ''MASCADA'', ''PANTALONVD'',
   ''PORTAFOLIO'', ''PORTATRAJE'', ''REGALOS'', ''SACO SPD'', ''SOLAPAS'', ''SOMBREROS'', ''UNIFORMES'')'
...
select Año as Año,
  ...
  FROM [ped_compr] where   Familia '+@FAMILIA2+'
  and MARCA '+@MARCAS2+'
  GROUP BY Año,
...
select Año as Año,categoria,''Importe''AS BASE,Familia,Colección as Marca,
  ...
  FROM VtaFamMesSOP
  where año>=2018 and Familia '+@FAMILIA2+' and [Colección] '+@MARCAS2+'
  GROUP BY Año,Categoria,Familia,Colección,
	   case when MES =1 then ''ENERO''when MES =2 then ''FEBRERO''when MES =3 then ''MARZO''when MES =4 then ''ABRIL''when MES =5 then ''MAYO''
			when MES =6 then ''JUNIO''when MES =7 then ''JULIO''when MES =8 then ''AGOSTO''when MES =9 then ''SEPTIEMBRE''when MES =10 then ''OCTUBRE''
			when MES =11 then ''NOVIEMBRE''when MES =12 then ''DICIEMBRE''end
