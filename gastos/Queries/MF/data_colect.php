...
REGIONAL,isnull( t.Correo,'na')as correo_tienda ,isnull( c.correo ,'na') as correo_manager from 
gastos_mf  as g left join Tienda  as t  on g.cod_almacen = t.Tienda COLLATE Modern_Spanish_CI_AS
...
$rawdata_colect=execonsultasqlsrv($sql_gastos,$conexionmf,$rawdata_colect);
$data_colect=array();
$dicc_tiendas=array();
$dicc_detalles=array();
$cabezera=array();
...
for ($i=0;$i<count($table_txt);$i++){
    ...
    //  IF($table_txt[$i][0]=='BM001'){
    emitepdf($css,$table_txt[$i][1],$table_txt[$i][0].'-'.$aact."-". strftime("%B", strtotime('-1month')));
    if($table_txt[$i][3]=="na"){
    ...
    $path_file='C:/laragon/www/developmenthh/PurePhpProyects/Gastos/Queries/MF/'.$table_txt[$i][0].'-'.$aact."-".  strftime("%B", strtotime('-1 month')). '.pdf';
